...
file 'update.rdf' => [XPI, 'install.rdf'] do
  ...
  update_rdf.xpath('//em:updateLink').each{|link| link.content = "https://raw.github.com/retorquere/zotero-#{EXTENSION}/master/#{XPI}" }
  update_rdf.xpath('//em:updateInfoURL').each{|link| link.content = "https://github.com/retorquere/zotero-#{EXTENSION}" }
...
file 'README.md' => [XPI, 'Rakefile'] do |t|
  ...
  readme.gsub!(/\(http[^)]+\.xpi\)/, "(https://raw.github.com/retorquere/zotero-#{EXTENSION}/master/#{XPI})")
  readme.gsub!(/\*\*[0-9]+\.[0-9]+\.[0-9]+\*\*/, "**#{RELEASE}**")
  File.open(t.name, 'w'){|f| f.write(readme)}
end

task :publish => [XPI, 'update.rdf', 'README.md'] do
  sh "git add --all ."
...
task :release, :bump do |t, args|
  ...
  install_rdf.at('//em:updateURL').content = "https://raw.github.com/retorquere/zotero-#{EXTENSION}/master/update.rdf"
